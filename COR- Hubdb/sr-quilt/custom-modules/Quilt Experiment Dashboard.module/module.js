(function () {
  ...
  const TABLE_COLUMNS = isSelectMetaUsable(_normalizedFormColumns)
...
`;
  ...
  function renderStats() {
    ...
    const success = NORMALIZED_DATA.filter(r => r._status === 'success').length;
    const failed = NORMALIZED_DATA.filter(r => r._status === 'failed').length;
  ...
  function renderRecentTable() {
    ...
    const tbody = $('#recent-experiments-body');
    if (!tbody) return;
    ...
    tbody.innerHTML = rows.length
      ...
      : '<tr><td colspan="10">No experiments yet.</td></tr>';
  ...
  function renderAllTable() {
    ...
    tbody.innerHTML = slice.length
      ...
      : '<tr><td colspan="10">No experiments match your filters.</td></tr>';
  ...
  function resetFilters() {
    ...
    ['filter-brand', 'filter-page-type', 'filter-test-type', 'filter-status'].forEach((id) => {
      const el = $('#' + id);
      if (el) el.value = '';
    });
    applyFilters();
  ...
  const REQUIRED_FIELDS = ['brand', 'landing_page', 'page_type', 'test_type', 'changes', 'hypothesis', 'owner', 'set_date'];
  ...
  function fillFormFromRow(row) {
    ...
    const selectKeys = ['brand', 'page_type', 'test_type', 'owner', 'status'];
  ...
  function bind() {
    ...
    $$('#filter-brand,#filter-page-type,#filter-test-type,#filter-status')
