...
/* ---------- HubDB SELECT resolution (inlined — Design Manager often omits sibling .js from require) ---------- */
...
async function fetchColumnsByName(client, tableId) {
    ...
    // Published table first (always valid). /draft GET may 404 on some portals.
...
exports.main = async (context, sendResponse) => {
    ...
    try {
        ...
        const encoded = encodeURIComponent(String(tableId).trim());
        const { data: createdRow } = await client.post(`/cms/v3/hubdb/tables/${encoded}/rows`, { values });

        sendResponse({
            body: { success: true, row: createdRow },
            statusCode: 200
        });
    } catch (err) {
        const status = err.response && err.response.status;
        const d = err.response && err.response.data;
        let message = (d && d.message) || err.message || 'Failed to create HubDB row.';
